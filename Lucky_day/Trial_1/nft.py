...
for event in events['asset_events']:
    if event_type == 'offer_entered'
        ...
        event_list.append([event['created_date'], bidder, float(bid_amount), event
        ['asset']['collection']['name'], event['asset']['token_id']])
...
response = r.json()
for asset in response["assests"]:
    if asset['name']:
        st.write(f"{asset['collection']['name']} #{asset['token_id']}")

    if asset['image_url'].endswith('mp4') or asset['image_url'].endswith
    ('mov'):
        st.video(asset['image_url'])
    else: 
        st.image(asset['image_url'])
